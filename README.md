...
4. Открыть [ссылку](https://raw.githubusercontent.com/moscow-technologies/ag-blockchain/master/parity/config/chain.json) и сохранить файл (`Ctrl+S`) в папку из предыдущего пункта. В появившемся окне "Сохранить как", поменять наименование файла на `chain.json` и сохранить файл. 
5. Открыть командную строку (нажать `Windows+R` и напечатать `cmd`, нажать `Выполнить`)
6. Перейти в папку с `parity.exe` командой `cd C:\Program Files\Parity\Parity`
